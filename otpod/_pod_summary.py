...
class PODSummary:
    ...
    *setVerbose*.
    ...
    def run(self):
        ...
        self._analysis = UnivariateLinearModelAnalysis(
            self._inputSample[:, 0],
            self._signals,
            self._noiseThres,
        ...
        if self._activeMethods["LinearGauss"]:
            ...
            self._PODgauss = UnivariateLinearModelPOD(
                self._inputSample[:, 0],
                self._signals,
                self._detection,
                self._noiseThres,
        ...
        if self._activeMethods["LinearBinomial"]:
            ...
            self._PODbin = UnivariateLinearModelPOD(
                self._inputSample[:, 0],
                self._signals,
                self._detection,
                self._noiseThres,
        ...
        if self._activeMethods["LinearKernelSmoothing"]:
            ...
            self._PODks = UnivariateLinearModelPOD(
                self._inputSample[:, 0],
                self._signals,
                self._detection,
                self._noiseThres,
        ...
        if self._activeMethods["QuantileRegression"]:
            ...
            self._PODqr = QuantileRegressionPOD(
                self._inputSample[:, 0],
                self._signals,
                self._detection,
                self._noiseThres,
        ...
        if self._activeMethods["PolynomialChaos"]:
            ...
            self._PODchaos = PolynomialChaosPOD(
                self._inputSample,
                self._signals,
                self._detection,
                self._noiseThres,
    ...
    def getResults(self, probabilityLevel=0.9, confidenceLevel=0.95):
        ...
        validationResult = "\n".join(
            [
                "{:<42} {:>10} {:>10} {:>10}".format(*line)
                for line in self._dataValidation
            ]
        )

        PODResult = "\n".join(
            ["{:<47} {:>13} {:>13}".format(*line) for line in self._dataPOD]
        )
    ...
    def saveResults(self, name, probabilityLevel=0.9, confidenceLevel=0.95):
        ...
        regressionResult = "\n".join(
            ["{}\t{}\t{}".format(*line) for line in self._analysis._dataRegression]
        )

        residualsResult = "\n".join(
            ["{}\t{}\t{}".format(*line) for line in self._analysis._dataResiduals]
        )

        validationResult = "\n".join(
            ["{}\t{}\t{}\t{}".format(*line) for line in self._dataValidation]
        )

        podResult = "\n".join(["{}\t{}\t{}".format(*line) for line in self._dataPOD])
    ...
    def _buildPrintResults(self, probabilityLevel, confidenceLevel):
        ...
        if (
            self._activeMethods["LinearGauss"]
            or self._activeMethods["LinearBinomial"]
        ...
        if self._activeMethods["QuantileRegression"]:
            # validation result
            quantRegValid = [
                "Quantile Regression at level "
                + str(round(probabilityLevel, 2))
                + " (> 0.6):",
                round(self._PODqr.getR2(probabilityLevel), n_digits),
                "",
                "",
            ]
        ...
        if self._activeMethods["PolynomialChaos"]:
            # validation result
            chaosValid = [
                "Polynomial Chaos (> 0.8):",
                round(self._PODchaos.getR2(), n_digits),
                round(self._PODchaos.getQ2(), n_digits),
                "",
            ]
        ...
        if self._dim > 1 and self._activeMethods["Kriging"]:
            ...
            krigingValid = [
                "Kriging (> 0.8):",
                "",
                round(self._PODkriging.getQ2(), n_digits),
                "",
            ]
    ...
    def drawGraphs(
        ...
        if self._boxCox:
            f, ax = self._analysis.drawBoxCoxLikelihood(
                os.path.join(directory, "BoxCox_likelihood.") + extension
            )
            fig.append(f)

        f, ax = self._analysis.drawLinearModel(
            model="uncensored",
            name=os.path.join(directory, "Linear_model.") + extension,
        )
        fig.append(f)
        f, ax = self._analysis.drawResiduals(
            model="uncensored", name=os.path.join(directory, "Residuals.") + extension
        )
        fig.append(f)
        f, ax = self._analysis.drawResidualsDistribution(
            model="uncensored",
            name=os.path.join(directory, "Residuals_distribution.") + extension,
        )
        fig.append(f)
        f, ax = self._analysis.drawResidualsQQplot(
            model="uncensored",
            name=os.path.join(directory, "Residuals_QQ_plot.") + extension,
        )
        fig.append(f)
        ...
        if self._censored:
            f, ax = self._analysis.drawLinearModel(
                model="censored",
                name=os.path.join(directory, "Linear_model_censored.") + extension,
            )
            fig.append(f)
            f, ax = self._analysis.drawResiduals(
                model="censored",
                name=os.path.join(directory, "Residuals_censored.") + extension,
            )
            fig.append(f)
            f, ax = self._analysis.drawResidualsDistribution(
                model="censored",
                name=os.path.join(directory, "Residuals_distribution_censored.")
                + extension,
            )
            fig.append(f)
            f, ax = self._analysis.drawResidualsQQplot(
                model="censored",
                name=os.path.join(directory, "Residuals_QQ_plot_censored.") + extension,
            )
            fig.append(f)

        if self._activeMethods["LinearGauss"]:
            try:
                f, ax = self._PODgauss.drawPOD(
                    probabilityLevel,
                    confidenceLevel,
        ...
        if self._activeMethods["QuantileRegression"]:
            try:
                f, ax = self._PODqr.drawPOD(
                    probabilityLevel,
                    confidenceLevel,
                    name=os.path.join(directory, "POD_Quantile_Regression.")
                    + extension,
                )
                fig.append(f)
            except:
            ...
            f, ax = self._PODqr.drawLinearModel(
                probabilityLevel,
                name=os.path.join(directory, "Quantile_regression_model.") + extension,
            )
            fig.append(f)
        if self._activeMethods["PolynomialChaos"]:
            try:
                f, ax = self._PODchaos.drawPOD(
                    probabilityLevel,
                    confidenceLevel,
            ...
            f, ax = self._PODchaos.drawValidationGraph(
                name=os.path.join(directory, "Validation_graph_Polynomial_chaos.")
                + extension
            )
            fig.append(f)
        if self._dim > 1 and self._activeMethods["Kriging"]:
            try:
                f, ax = self._PODkriging.drawPOD(
                    probabilityLevel,
                    confidenceLevel,
            ...
            f, ax = self._PODkriging.drawValidationGraph(
                name=os.path.join(directory, "Validation_graph_Kriging.") + extension
            )
            fig.append(f)
