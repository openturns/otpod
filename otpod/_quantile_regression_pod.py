...
class QuantileRegressionPOD(POD):
    ...
    **Available constructor:**
    ...
    def __init__(
        ...
        super(QuantileRegressionPOD, self).__init__(
        ...
        if self._censored:
            logging.info(
                "Censored data are not taken into account : the quantile "
    ...
    def run(self):
        ...
        for probLevel in self._quantile:
            ...
            model = self._buildModel(1.0 - probLevel)
            # Solve the model == detectionBoxCox with defects
            # boundaries = [0, defectMax]
            defectList.append(
                ot.Brent().solve(model, self._detectionBoxCox, 0, defectMax)
            )
        ...
        yvalue = np.hstack([0.0, self._quantile, self._quantile.max()])
        ...
        for i in range(self._simulationSize):
            ...
            defectList = []
            for probLevel in self._quantile:
                fit = algoQuantReg.fit(1.0 - probLevel, max_iter=300, p_tol=1e-2)
                ...
                defectList.append(
                    ot.Brent().solve(
                        model, detectionBoxCox, -ot.SpecFunc.MaxScalar, defectMax
                    )
                )
    ...
    def computeDetectionSize(self, probabilityLevel, confidenceLevel=None):
        ...
        defectMax = self._defects.getMax()[0]
        # compute 'a90'
        model = self._buildModel(1.0 - probabilityLevel)
        try:
    ...
    def drawLinearModel(self, probabilityLevel, name=None):
        ...
        model = self._algoQuantReg.fit(1.0 - probabilityLevel)
        ...
        ax.set_title(
            "Quantile regression model at level (1 - " + str(probabilityLevel) + ")"
        )
        ax.grid()
